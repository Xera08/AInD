...
export function characterRules(character: { name: string; className: string; race: string; level: number; inventory: string[] }): string {
  ...
  return `CHARACTER RULES (SRD 5e): ${character.name} is a level ${character.level} ${character.race} ${character.className}. ${classRule} Starting inventory: ${character.inventory.join(', ')}. The inventory is authoritative: the character can only use, draw, give, or pick up an item if it is in this inventory or the item is visibly acquired in the fiction first. Do not create a missing item just because the player names it. Enforce level-gated features and spell access. If an action is unavailable, explain the fiction naturally and ask what they do instead. Never silently grant a class feature or item.`;
}
